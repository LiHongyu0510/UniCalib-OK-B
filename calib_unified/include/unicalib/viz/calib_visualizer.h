...
struct VizConfig {
    ...
    // ========== 交互式手动调整配置 ==========
    bool enable_interactive = true;        // 启用交互式调整
    bool enable_keyboard = true;           // 启用键盘控制
    bool enable_mouse = true;              // 启用鼠标控制
    
    // 旋转模式
    enum RotationEditMode { 
        EULER = 0,           // 欧拉角 (roll/pitch/yaw)
        AXIS_ANGLE = 1,     // 轴角 (axis + angle) - 推荐
        QUATERNION = 2,      // 四元数
        ROTATION_VECTOR = 3  // 旋转向量 (与BA兼容)
    };
    RotationEditMode rotation_mode = AXIS_ANGLE;  // 默认轴角模式
    
    // 调整步长
    double trans_step_m = 0.01;     // 平移步长 [m]
    double rot_step_deg = 0.5;       // 旋转步长 [度]
    double fast_multiplier = 10.0;   // 快速模式倍数 (Shift键)
    
    // 精度阈值
    double good_threshold_px = 1.0;  // 优秀阈值 [像素]
    double warn_threshold_px = 3.0;  // 警告阈值 [像素]
    double good_threshold_m = 0.01;  // 优秀阈值 [米]
...
public:
    ...
    /**
     * @brief 将一帧 LiDAR 点云按外参投影到同时间戳图像上并同时展示（弹窗 + 可选保存）
     * @param cloud 点云
     * @param image 同帧图像
     * @param T_cam_lidar 相机相对 LiDAR 的外参（点从 LiDAR 系变换到相机系：p_cam = T_cam_lidar * p_lidar）
     * @param intrin 相机内参
     * @param save_path 保存路径，空则仅展示不保存（非空时也会 imshow）
     * @param display_ms 弹窗显示时长（毫秒），≤0 表示不自动关闭，需用户按键
     */
    void show_lidar_projection_on_image(
    ...
    /**
     * @brief 启动 Pangolin 显示窗口（创建并绑定 OpenGL 上下文）
     * 标定前调用以便实时可视化有窗口可绘制；无头环境可跳过。
     */
    void start_display(const std::string& title = "UniCalib Visualization",
                       int width = 1280, int height = 720);

    /**
     * @brief 关闭 Pangolin 显示窗口（标定流程结束后调用，界面自动关闭）
     */
    void close_display();
    ...
    // 回调函数类型
    using ExtrinsicChangedCallback = std::function<void(const ExtrinsicSE3&)>;
     ...
     * @brief 启动交互式外参调整模式
     */
    std::optional<ExtrinsicSE3> run_interactive_adjustment(
    ...
    void set_on_extrinsic_changed(ExtrinsicChangedCallback cb);
    void set_on_accept(AcceptCallback cb);
    void set_on_cancel(CancelCallback cb);
    void set_rotation_mode(VizConfig::RotationEditMode mode);
    VizConfig::RotationEditMode get_rotation_mode() const;
    void translate(double dx, double dy, double dz);
    void rotate(double rx, double ry, double rz);
    bool undo();
    bool redo();
    void reset_to_initial();
    std::optional<ExtrinsicSE3> get_current_extrinsic() const;
    bool start_video_recording(const std::string& output_path, int fps = 30);
    void stop_video_recording();
    void save_frame_to_video();
    void update_realtime_data(
        const pcl::PointCloud<pcl::PointXYZI>::Ptr& ref_cloud,
        const pcl::PointCloud<pcl::PointXYZI>::Ptr& target_cloud,
        const ExtrinsicSE3& extrinsic);
    bool render_and_handle_input();
...
private:
    ...
    // Pangolin 3D 渲染器 (仅在 UNICALIB_WITH_PANGOLIN 时启用)
#if UNICALIB_WITH_PANGOLIN
    PangolinRender::Ptr pangolin_render_;
#endif

    // 实时数据缓存
    pcl::PointCloud<pcl::PointXYZI>::Ptr ref_cloud_;
    pcl::PointCloud<pcl::PointXYZI>::Ptr target_cloud_;
    ExtrinsicSE3 current_extrinsic_;

    // 外参调整历史 (用于 undo/redo)
    std::vector<ExtrinsicSE3> history_;
    int history_index_ = -1;
    static constexpr int MAX_HISTORY = 50;

    // 回调函数
    ExtrinsicChangedCallback extrinsic_changed_cb_;
